...
/**
 * Number in Bangladeshi style
 *
 * PHP Library
 *
 * @author Sharif Uddin<sharif@probortonapps.com>
 */
class Banglanumber {
    /**
     * Hundred Word
    ...
    public function numToWord( $number ) {
        ...
        if ( ! is_numeric( $number ) ) {
            return 'Error: Valid number not given.' ;
        }
        // Check if number is exceeding the system maximum
        if ( $number > PHP_INT_MAX ) {
            return 'Error: Number is greater than system maximum';
        }
        ...
        if ( is_float( $number ) ) {
            ...
            if ( isset( $dot[1] ) && $dot[1] > 0 ) {
                ...
                return $this->convertNumber( $dot[0] ) . ' ' . $this->taka . ' ' . $this->and . ' ' . $this->convertNumber(substr($dot[1], 0, 2)) .' '.$this->paisa;
            } else {
                return $this->convertNumber( $dot[0] ). ' '.$this->taka;
            }
        }
    ...
    public function numToWordSmall( $number ) {
        ...
        if ( $number > 99 ) {
            return 'Error: Number is greater than 99. Use numToWord method.';
        }
